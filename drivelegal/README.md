...
- Weather condition and temperature shown in drive mode UI
...
### Dual HUD Modes
- **Car Display Mode** (`/car`): Dashboard-style 16:9 layout, large speedometer, voice-only — designed for Android Auto / CarPlay
- **Phone Drive Mode** (`/drive`): Vertical high-contrast interface for dashboard mounting, real OBD-II speed integration, waveform voice UI
...
| `/drive` | Phone driving mode |
| `/car` | Car display simulator (Android Auto style) |
...
│   │   ├── car/               # Car display HUD (AI alert banner)
│   │   ├── drive/             # Phone driving mode (OBD-II + GPS + AI alerts)
...
│   │   ├── OnboardingFlow.tsx # 5-step onboarding wizard
│   │   ├── Robot3D.tsx        # 3D robot wrapper
...
5. Tap **"Connect OBD-II Adapter"** — Chrome will show a BLE device picker
6. Select your adapter from the list
7. The speed display will switch from GPS to **OBD-II Live** (green indicator)
...
5. GPS and microphone permission request
